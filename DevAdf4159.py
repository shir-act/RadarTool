...
class DevAdf4159(DevDriver.DevDriver):
    ...
    def     DevSetReg(self, Regs):
        ...
        if self.USpiCfg_Type == 'USpi':
            ...
            Ret     =   self.Brd.Fpga_SetUSpiData(dUSpiCfg, Regs)
        return Ret
    ...
    def     GenRegs(self):
        ...
        NStrt               =   fStrtDiv/fPfd
        NStrtInt            =   np.floor(NStrt)
        NStrtFracMsb        =   np.floor((NStrt - NStrtInt) * 2**12)
        NStrtFracLsb        =   np.floor(((NStrt - NStrtInt) * 2**12 - NStrtFracMsb) * 2**13)

        NStop               =   fStopDiv/fPfd
        NStopInt            =   np.floor(NStop)
        NStopIntMsb         =   np.floor((NStop - NStopInt) * 2**12)
        NStopIntLsb         =   np.floor(((NStop - NStopInt) * 2**12 - NStopIntMsb) * 2**13)

        DevMax              =   2**15
        fRes                =   fPfd/(2**25)
        DevOffs             =   np.ceil(np.log10(self.fDev/(fRes*DevMax))/np.log10(2))
        fDevRes             =   fRes * 2**DevOffs
        DevVal              =   np.round(self.fDev/(fRes * 2**DevOffs))

        fDevVal             =   fPfd/(2**25) * (DevVal * 2**DevOffs)

        NrSteps             =   np.ceil(BandwidthDiv/fDevVal)                                      # number of steps needed to cover bandwith
        tStep               =   self.TRampUp/NrSteps                                            # time between hops
        Clk2                =   1                                                               # choosen value
        Clk1                =   np.round(tStep * fPfd/Clk2)

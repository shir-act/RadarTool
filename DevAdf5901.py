...
class DevAdf5901(DevDriver.DevDriver):
    ...
    def     DevSetReg(self, Regs):
        ...
        if self.USpiCfg_Type == 'USpi':
            ...
            self.Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[0:13])
            self.Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[13:15])
            self.Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[15:17])
            Ret                 =   self.Brd.Fpga_SetUSpiData(dUSpiCfg, Regs[17:19])
    ...
    def     GenRegs(self):
        ...
        DivInt          =   np.floor(Div)
        DivFrac         =   np.round((Div - DivInt) * 2**25)
